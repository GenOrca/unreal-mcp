...
ACTION_METADATA = {'add_rig_bone': {'asset_path_params': ['asset_path'],
                  'description': 'Adds a bone to a Control Rig hierarchy under an optional parent '
                                 '(requires the ControlRig plugin).',
                  'effect': 'write',
                  'idempotent': False,
                  'required_plugins': ['ControlRig'],
 ...
 'add_rig_null': {'asset_path_params': ['asset_path'],
                  'description': 'Adds a null (group transform) to a Control Rig hierarchy '
                                 '(requires the ControlRig plugin).',
                  'effect': 'write',
                  'idempotent': False,
                  'required_plugins': ['ControlRig'],
                  ...
                  'title': 'Add Rig Null',
                  'ue_versions': ['5.6', '5.7', '5.8']},
 'add_unit_node': {'asset_path_params': ['asset_path', 'struct_path'],
                   'description': 'Adds a RigVM unit node by struct path (e.g. '
                                  "'/Script/ControlRig.RigUnit_GetTransform') to a Control Rig "
                                  'graph (requires the ControlRig plugin).',
                   'effect': 'write',
                   'idempotent': False,
                   'required_plugins': ['ControlRig'],
                   'requires_confirmation': True,
                   ...
                   'title': 'Add Unit Node',
                   'ue_versions': ['5.6', '5.7', '5.8']},
 'create_control_rig': {'asset_path_params': ['asset_path', 'skeletal_mesh_path'],
                        'description': 'Creates a Control Rig at asset_path; with a skeletal mesh, '
                                       'imports its bones and sets it as preview (requires the '
                                       'ControlRig plugin).',
                        'effect': 'write',
                        'idempotent': False,
                        'required_plugins': ['ControlRig'],
                        ...
                        'ue_versions': ['5.6', '5.7', '5.8']},
 'get_control_rig_info': {'asset_path_params': ['asset_path'],
                          'description': 'Returns element counts by type and the preview mesh of a '
                                         'Control Rig (requires the ControlRig plugin).',
                          'effect': 'read',
                          'idempotent': True,
                          'required_plugins': ['ControlRig'],
                          ...
                          'supports_undo': False,
                          'title': 'Get Control Rig Info',
                          'ue_versions': ['5.6', '5.7', '5.8']},
 'recompile_control_rig': {'asset_path_params': ['asset_path'],
                           'description': "Recompiles a Control Rig's VM and saves it (requires "
                                          'the ControlRig plugin).',
                           'effect': 'write',
                           'idempotent': False,
                           'required_plugins': ['ControlRig'],
                           'requires_confirmation': True,
